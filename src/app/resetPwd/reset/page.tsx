...
async function validateToken(token: string | null) {
  ...
  try {
    const response = await fetch(`${process.env.NEXTAUTH_URL}/api/reset-password/validate`, {
      method: 'POST',
...
export default async function ResetPasswordPage({ params }: any) {
  ...
  const updatePassword = async (formData: FormData) => {
    ...
    if (password !== confirmPassword) {
      return redirect('/resetPwd/reset?error=password_mismatch&token=' + token);
    }

    if (typeof password !== 'string' || password.length < 6) {
      return redirect('/resetPwd/reset?error=invalid_password&token=' + token);
    }

    try {
      const validation = await fetch(`${process.env.NEXTAUTH_URL}/api/reset-password/validate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token }),
        cache: 'no-store'
      ...
      if (!validation.ok) {
        return redirect('/resetPwd/reset?error=invalid_token');
      }

      const updateResponse = await fetch(`${process.env.NEXTAUTH_URL}/api/reset-password/update`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!updateResponse.ok) {
        ...
        return redirect(`/resetPwd/reset?error=${encodeURIComponent(error.message)}&token=${token}`);
      }

      return redirect('/login');

    } catch (error) {
      return redirect(`/resetPwd/reset?error=connection_error&token=${token}`);
    }
  };
