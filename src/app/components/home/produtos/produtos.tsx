...
export default function Produtos() {
  return (
    <div className="mx-auto py-10 px-4 sm:px-0">
      ...
      >
        ...
        <CarouselContent className="flex gap-[1px] px-3">
          {produtos.map((produto) => (
            <CarouselItem
              key={produto.id}
              className="flex-shrink-0 basis-1/4 pl-2 [@media(max-width:992px)]:basis-[60%] [@media(max-width:600px)]:basis-[85%]"
            >
              <Link
                href={`/products/${produto.nome.toLowerCase().replace(/\s+/g, "-")}`}
