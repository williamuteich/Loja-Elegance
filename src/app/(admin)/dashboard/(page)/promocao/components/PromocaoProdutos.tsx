...
type PromoAction = (data: any) => Promise<{ success?: string; error?: string }>;
...
export default function PromocaoProdutos({
  ...
  useEffect(() => {
    // Verifica se está no cliente (não executa durante SSR)
  ...
  const removerProdutoPromocao = async (index: number) => {
    ...
    try {
      const result = await promoAction({
        actionType: 'revert',
        produtoId: promo.produto.id 
      });
