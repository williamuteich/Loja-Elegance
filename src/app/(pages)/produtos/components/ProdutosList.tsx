...
export default function ClientProdutos({
  ...
  const [paginaAtual, setPaginaAtual] = useState(Number(searchParams.page) || 1);
  ...
  useEffect(() => {
    ...
    if (
      prevFiltros.search !== filtros.search ||
      prevFiltros.categoria !== filtros.categoria ||
      prevFiltros.precoMin !== filtros.precoMin ||
      prevFiltros.precoMax !== filtros.precoMax
    ) {
      setPaginaAtual(1);
    }
  ...
  const atualizarPagina = (novaPagina: number) => {
    ...
    router.push(`/produtos?${new URLSearchParams({
      ...filtros,
