...
export default function ViewImages({ produtos }: { produtos: any }) {
  ...
  return (
    <div className="flex flex-col-reverse lg:flex-col xl:flex-row gap-4 items-center xl:items-start w-full">
      ...
      <div
        className="
          flex flex-nowrap lg:flex-row xl:flex-col gap-2 items-center 
          justify-start 
          overflow-x-auto xl:overflow-x-hidden 
          overflow-y-hidden xl:overflow-y-auto 
          w-full xl:w-auto xl:max-h-[500px] custom-scrollbar
          mb-4 lg:mb-0 px-1
        "
      ...
      >
        {imagesToDisplay.map((img: string, index: number) => (
          <div
            key={index}
            className="
              h-[90px] w-[90px] lg:h-[100px] lg:w-[100px] xl:h-[125px] xl:w-[125px] 
              rounded cursor-pointer hover:scale-105 transition-transform 
              flex items-center justify-center border border-gray-200 flex-shrink-0
              min-w-[90px]
            "
            onClick={() => {
              setLoading(true)
